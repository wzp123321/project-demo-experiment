...
const getModelAssigner = (vnode: VNode) => {
  const fn = (vnode as any).props["onUpdate:modelValue"];
...
const registerInputFilter = (app: App) => {
  app.directive("textInputFilter", {
    created(el, { modifiers }, vnode) {
      ...
      console.log(vnode, getModelAssigner(vnode));
      ...
      addEventListener(ele, "input", (e) => {
        ...
        const domValue = ele.value;
        // console.log(domValue, ele);
        ele.value = domValue;
